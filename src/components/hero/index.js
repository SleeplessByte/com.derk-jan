...
export default class Hero extends Component {
	...
	state = {
		...
		needFixedViewportHeight: /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent),
	...
	onResized() {
		...
		if (Math.abs(this.state.viewportHeight - window.innerHeight) > HEIGHT_CHANGE_TOLERANCE) {
			this.setState({
				viewportHeight: window.innerHeight,
	...
	render() {
		...
		const elementStyle = this.state.elementHeight === undefined
			? {}
			: { height: `${this.state.elementHeight}px` }

		return (
			<div class={classNameHero} ref={(_) => this.element = _} style={elementStyle}>
